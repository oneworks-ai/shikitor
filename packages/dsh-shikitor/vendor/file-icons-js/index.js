...
const icondb = [
  ...
  ["config-icon", ["medium-yellow", "medium-yellow"], /\.(?:ini|desktop|directory|cfg|conf|prefs)$|MANIFEST\.in$/i, , false, , /\.ini$/i, /^d[0o]sini$/i],
...
function collectIconClasses(value, output) {
  if (!Array.isArray(value)) return
  if (typeof value[0] === 'string' && value[0].endsWith('-icon')) output.add(value[0])
  for (const child of value) collectIconClasses(child, output)
}

const discoveredIconClasses = new Set()
collectIconClasses(icondb, discoveredIconClasses)

/** All Atom File Icons glyph classes, exposed for host-side icon pickers. */
export const iconClasses = Object.freeze([...discoveredIconClasses].sort())
 ...
 * @public
 */
export const getClass = (name, match = db.matchName(name)) => match ? match.getClass() : null
 ...
 * @public
 */
export const getClassWithColor = (name, match = db.matchName(name)) => match ? match.getClass(0) : null
